...
def render_table1(profile: dict) -> Panel:
    ...
    return Panel(
        content,
        title=(
            f"[bold {BLUE}]🔬  DNA ANALYSIS[/bold {BLUE}]  [{GREY}]{short}  ·  {label}[/{GREY}]"
        ),
...
def render_cluster_drilldown(cluster: dict, profiles: list[dict]) -> Panel:
    ...
    intro.append("  Average similarity: ", style=GREY)
    intro.append(f"{avg_sim:.3f}", style=f"bold {RED if avg_sim >= 0.92 else AMBER}")
    intro.append("  ·  Threshold for same operator: 0.92\n", style=GREY)
    if avg_sim >= 0.92:
        intro.append(
            "\n  These wallets show near-identical behaviour across all 7 dimensions.\n",
            style=f"bold {RED}",
        )
        intro.append("  Different addresses — same hand operating them.\n", style=AMBER)
    else:
        intro.append("\n  These wallets share significant behavioural overlap.\n", style=AMBER)
        intro.append(
            "  May be the same operator or wallets following a similar strategy.\n", style=GREY
        )
...
async def _quick_lookup_fetch(address: str, chain: str) -> Optional[dict]:
    ...
    try:
        ...
        ) or sum(float(t.value_native) for t in txs if t.value_native and not t.token_symbol)
        stable_usd = sum(
            float(t.value_native)
            for t in txs
            if t.value_native
        ...
        usd_price = 0.0
        try:
            coin_id = {"ETH": "ethereum", "TRX": "tron", "DOGE": "dogecoin"}.get(
                chain.upper(), "ethereum"
            )
        ...
        return {
            ...
            "wallet_class": profile.classification.wallet_class.value
...
def quick_lookup() -> None:
    ...
    console.print()
    console.print(
        f"  [{GREY}]Paste any address — ETH, TRX, or DOGE.  Nothing will be saved.[/{GREY}]"
    )
    console.print()
    ...
    if not profile:
        console.print(
            f"  [{AMBER}]Could not fetch data — check API key or address has transactions.[/{AMBER}]"
        )
        console.input(f"\n  [{GREY}]Press Enter to continue...[/{GREY}]")
        return
...
def prompt_batch_addresses() -> list[dict]:
    console.print()
    console.print(
        f"  [{GREY}]Paste addresses — one per line, mixed chains OK, blank line to finish.[/{GREY}]"
    )
    ...
    while True:
        ...
        console.print(
            f"  [{GREEN}]  ✓ {chain}  {addr[:12]}...{addr[-6:]}[/{GREEN}]  [{GREY}]{label}[/{GREY}]"
        )
        idx += 1
...
def prompt_case_open_or_create(manager: CaseManager) -> Optional[str]:
    ...
    if cases:
        ...
        console.print(
            f"  [{GREY}]No cases found.  Enter a name to create your first case.[/{GREY}]"
        )
        console.print(
            f"  [{GREY}]Or press [bold]L[/bold] for a quick one-off address lookup.[/{GREY}]"
        )
        console.print()
...
def case_menu(manager: CaseManager, case_name: str) -> None:
    ...
    while True:
        ...
        if cluster_list:
            cl_summary = "  ".join(f"{cl['label']} {cl['member_count']}w" for cl in cluster_list)
        ...
        console.print(
            f"  [{BLUE}][R][/{BLUE}]  Re-analyse all  [{GREY}](live API, ignores cache)[/{GREY}]"
        )
        ...
        if cluster_list:
            console.print(
                f"  [{BLUE}][X][/{BLUE}]  [{RED}]Cluster drill-down  [{GREY}]({len(cluster_list)} cluster{'s' if len(cluster_list) != 1 else ''} detected)[/{GREY}][/{RED}]"
            )
